...
export async function GET(req: NextRequest) {
  ...
  const state = verifyOAuthState(req.nextUrl.searchParams.get("state"), "fitbit");

  if (!code || !state) {
    return NextResponse.redirect(new URL("/settings?error=fitbit", req.url));
  }
  ...
  if (!tokenRes.ok) {
    return NextResponse.redirect(new URL("/settings?error=fitbit_token", req.url));
  }

  const tokenData = await tokenRes.json();
  const admin = createAdminSupabaseClient();
  const displayName = typeof tokenData.user_id === "string" ? `Fitbit ${tokenData.user_id}` : "Fitbit";
  ...
  if (error) {
    return NextResponse.redirect(new URL("/settings?error=fitbit_save", req.url));
  }

  return NextResponse.redirect(new URL("/settings?connected=fitbit", req.url));
}
