...
export const metadata: Metadata = {
  title: "End User Licence Agreement | Dad Health",
...
/** Strip Termly / Word export chrome; keep full text and links. Styling matches privacy / terms. */
const sanitizeEulaHtml = (html: string) => {
  let out = html;
  ...
  out = out.replace(/<a[^>]*href="https:\/\/app\.termly\.io\/dsar\/[^"]*"[^>]*>[\s\S]*?<\/a>/gi, "");
  ...
  out = out.replace(/<h1>[\s\S]*?END USER\s+LICEN[CS]E\s+AGREEMENT[\s\S]*?<\/h1>/gi, "");
...
const getEulaBodyHtml = async () => {
  ...
  const cleaned = sanitizeEulaHtml(inner);
  return wrapYesNoTableCells(cleaned);
};

const articleClassName =
  "max-w-none text-muted-foreground text-[15px] leading-7 [&_h1]:font-heading [&_h1]:text-[30px] [&_h1]:leading-tight [&_h1]:font-extrabold [&_h1]:uppercase [&_h1]:tracking-wide [&_h1]:!text-primary [&_h2]:font-heading [&_h2]:text-[28px] [&_h2]:leading-tight [&_h2]:font-extrabold [&_h2]:uppercase [&_h2]:tracking-wide [&_h2]:!text-primary [&_h3]:font-heading [&_h3]:text-[20px] [&_h3]:font-extrabold [&_h3]:uppercase [&_h3]:tracking-wide [&_h3]:!text-primary [&_p]:my-3 [&_li]:my-1 [&_ul]:list-disc [&_ul]:pl-6 [&_ol]:list-decimal [&_ol]:pl-6 [&_table]:w-full [&_table]:min-w-[780px] [&_table]:text-sm [&_table]:border [&_table]:border-border [&_table]:rounded-sm [&_table]:overflow-hidden [&_table]:[border-collapse:collapse] [&_th]:px-4 [&_th]:py-3 [&_th]:!text-left [&_th]:font-heading [&_th]:text-[10px] [&_th]:font-extrabold [&_th]:tracking-[1.8px] [&_th]:uppercase [&_th]:!text-primary [&_th]:bg-muted/40 [&_td]:px-4 [&_td]:py-3 [&_td]:align-top [&_td]:tracking-normal [&_td]:leading-7 [&_td]:normal-case [&_td]:!tracking-normal [&_td]:!leading-7 [&_td]:![letter-spacing:0px] [&_td]:![word-spacing:0px] [&_td]:![text-rendering:auto] [&_td]:[text-align:left] [&_td]:[text-justify:auto] [&_td]:[font-kerning:none] [&_td]:break-words [&_td]:whitespace-normal [&_td_*]:!tracking-normal [&_td_*]:![letter-spacing:0px] [&_td_*]:![word-spacing:0px] [&_tr]:border-b [&_tr]:border-border [&_strong]:text-foreground [&_a]:text-primary [&_a]:no-underline [&_a]:decoration-transparent [&_a]:transition-all [&_a]:duration-200 [&_a:hover]:underline [&_a:hover]:underline-offset-4 [&_a:hover]:decoration-[hsl(78,89%,65%)]";

const EulaPage = async () => {
  const policyHtml = await getEulaBodyHtml();
    ...
    <SitePageShell>
      <section className="bg-primary text-primary-foreground">
        <div className="max-w-[1400px] mx-auto px-5 lg:px-8 py-12">
          ...
          <div className="flex items-end justify-between flex-wrap gap-4">
            <h1 className="font-heading text-[28px] sm:text-[36px] lg:text-[48px] font-extrabold uppercase leading-none tracking-wide max-w-[min(100%,52rem)]">
      ...
      <section className="bg-background">
        <div className="max-w-[1400px] mx-auto px-5 lg:px-8 py-8 lg:py-10">
          <article className={articleClassName} dangerouslySetInnerHTML={{ __html: policyHtml }} />
        </div>
      </section>
