...
export default function SettingsPage() {
  ...
  const handleMasterToggle = async (next: boolean) => {
    ...
    try {
      ...
      console.error("[settings] updateProfile failed", e);
      toast({ description: "Could not update settings.", variant: "destructive" });
    }
  };
  ...
  const handleTypeToggle = async (type: NotificationType, enabled: boolean) => {
    ...
    try {
      ...
      console.error("[settings] upsertPref failed", e);
      toast({ description: "Could not update notification preference.", variant: "destructive" });
    }
  };
  ...
  const handleTimeChange = async (type: NotificationType, hhmm: string) => {
    ...
    try {
      ...
      console.error("[settings] upsertPref time failed", e);
      toast({ description: "Could not save time.", variant: "destructive" });
    }
  };
  ...
  const handleTimezoneSave = async () => {
    ...
    try {
      ...
      console.error("[settings] timezone save failed", e);
      toast({ description: "Could not save timezone.", variant: "destructive" });
    }
  };
  ...
  return (
    <SitePageShell>
      <section className="bg-background border-b border-border">
        <div className="w-full px-5 lg:px-8 py-10 max-w-3xl">
          ...
          <div className="mt-6 flex items-center justify-between gap-4 rounded-lg border border-border bg-card px-4 py-3">
            ...
            <Switch checked={pushEnabled} onCheckedChange={handleMasterToggle} />
          </div>
          ...
          <div className="mt-4 rounded-lg border border-border bg-card px-4 py-3">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="shrink-0 inline-flex items-center justify-center rounded-md border border-input bg-background px-3 py-2 text-xs font-semibold hover:bg-accent"
              >
