...
export function getDashboardScore(dashboard: DashboardData | null | undefined, hasUser: boolean): number | string {
  if (typeof dashboard?.total_score === "number") return Math.round(dashboard.total_score);
  if (dashboard?.total_score != null) return dashboard.total_score;
  return hasUser ? 74 : "—";
}
...
export function getMoodWeek(moodLogs: MoodLog[], dayKeys: string[]) {
  ...
  return dayKeys.map((d) => moodMap.get(d) ?? 3);
}
...
export function getMoodSummary(moodWeek: number[], hasUser: boolean) {
  ...
  const scoreText = moodWeek.some((v) => v > 0) ? ` (${avg.toFixed(1)}/4)` : "";
